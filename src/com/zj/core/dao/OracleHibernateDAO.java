...
public class OracleHibernateDAO extends CommonDAOImpl {
	private static final Log log = LogFactory.getLog(OracleHibernateDAO.class);
	@SuppressWarnings("unchecked")
	@Override
	public <T> PageInfo<T> querySQL(String sql,int pageSize,int pageNum,String dbname){
		log.info(">>>>>>>start OracleHibernateDAO.querySQL()...");
        ...
        if(usedtime > 3000){
        	log.warn("OracleHibernateDAO.querySQL() used time=" + usedtime + " s");
        }
        return pageinfo;
	}
}
